...
import { CoreModule, isProd } from 'nestjs-shared/core';
import { DatabaseModule } from 'nestjs-shared/database';
import { ObservabilityModule } from 'nestjs-shared/observability';
import { GOOGLE_SCOPES, TokenVaultModule } from 'nestjs-shared/token-vault';
import { DmsModule } from 'nestjs-shared/dms';
import { CustomFieldsModule, CustomFieldType } from 'nestjs-shared/custom-fields';
import { JsonStoreModule } from 'nestjs-shared/json-store';
import { QueueModule } from 'nestjs-shared/queue';
import { CronModule } from 'nestjs-shared/cron';
import { PrismaClient } from '../generated/prisma/client';
import { Configkey } from './shared/config-keys';
import { AuthModule } from 'nestjs-shared/auth';
import { CommentModule } from 'nestjs-shared/comment';
import { CorrespondenceModule } from 'nestjs-shared/correspondence';
...
function isTokenVault2Enabled(): boolean {
  return (process.env[Configkey.APP_SECRET]?.length ?? 0) >= 32;
}

function isDms2Enabled(): boolean {
  return isConfiguredEnv(process.env[Configkey.FIREBASE_CREDENTIAL]);
}
    ...
    TokenVaultModule.forRootAsync({
      ...
      useFactory: (cfg: ConfigService) => ({
        ...
        googleOAuth: {
          ...
          callbackUrl: 'auth/oauth2/google/callback',
          allowedScopes: [
