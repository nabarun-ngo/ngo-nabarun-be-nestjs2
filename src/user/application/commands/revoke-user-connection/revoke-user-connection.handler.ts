...
@CommandHandler(RevokeUserConnectionCommand)
@Injectable()
export class RevokeUserConnectionHandler
  implements ICommandHandler<RevokeUserConnectionCommand, void>
{
  constructor(
  ...
  async execute({ params: cmd }: RevokeUserConnectionCommand): Promise<void> {
    ...
    await this.identityProvider.revokeConnection(user.idpSub, cmd.connectionKey);
  }
}
